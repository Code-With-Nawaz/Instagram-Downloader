...
function InstagramUserInfo() {
  ...
  const handleFetchData = async () => {
    ...
    try {
      const {
        ...
        data,
        error: apiError,
      } = await fetchInstagramUserData(username);
  ...
  return (
    <>
      <Helmet>
        <title>Instagram User Information - Profile Insights</title>
        <meta
          name="description"
          content="Get detailed information about Instagram users, including follower count, bio, and more."
        />
        <meta
          name="keywords"
          content="Instagram, User Information, Profile Insights, Follower Count, Bio Information"
        />
      </Helmet>
      ...
      <main className="flex-grow min-h-screen flex items-center justify-center p-4 sm:p-6 bg-gradient-to-r from-blue-600 via-indigo-700 to-purple-800">
        <div className="slide-in-bottom bg-white rounded-lg p-6 sm:p-8 shadow-2xl w-full max-w-md sm:max-w-lg">
          <h1 className="text-3xl sm:text-4xl font-extrabold text-center mb-6 text-gray-900">
            Instagram User Info
          </h1>
          <p className="text-center text-gray-600 mb-6">
            Enter the Instagram username below to fetch user information.
          </p>
          <input
            type="text"
          ...
          {userData ? (
            <div className="mt-6 text-center">
              ...
              <p className="text-md text-gray-500 mt-2">
                Category: {userData.category_name || "Not available"}
              </p>
              <p className="text-md text-gray-500 mt-2">
                Bio Links:
              ...
              <p className="text-md text-gray-500 mt-2">
                About: {userData.biography || "Not available"}
              </p>
            </div>
          ) : (
